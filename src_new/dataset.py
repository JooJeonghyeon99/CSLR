...
class AVHubertDataset_mvsr(FairseqDataset):
    ...
    def collater(self, samples):
        ...
        ratio_list = [s.get('fr_ratio', None) for s in samples]
        ...
        if self.is_s2s and prev_output_tokens_list:
            seq_label_ptr = 0
            for label_idx, label_rate in enumerate(self.label_rates):
                if label_rate != -1:
                    continue
                if seq_label_ptr >= len(prev_output_tokens_list):
                    break
                prev_tokens = prev_output_tokens_list[seq_label_ptr]
                lengths = lengths_list[label_idx]
                if seq_label_ptr >= len(token_lang_ids_list):
                    break
                token_lang_tensor = self._build_token_language_tensor(
                    prev_tokens, lengths, lang_list, ratio_list
                )
                token_lang_ids_list[seq_label_ptr] = token_lang_tensor
                seq_label_ptr += 1
        ...
        if self.single_target:
            ...
            if self.is_s2s and prev_output_tokens_list:
                ...
                if token_lang_ids_list and token_lang_ids_list[0] is not None:
    ...
    def _build_token_language_tensor(self, prev_output_tokens, lengths, lang_list, ratio_list):
        if prev_output_tokens is None or lengths is None:
            return None
        device_tensor = prev_output_tokens
        B, L = device_tensor.size()
        token_lang_ids = device_tensor.new_zeros((B, L))

        lang_code_to_id = {"en": 0, "it": 1, "fr": 2, "es": 3, "pt": 4}
        cs_first, cs_second = (self.cs_pair_ids if self.cs_pair_ids is not None else (None, None))

        for b in range(B):
            base_lang_id = lang_code_to_id.get(lang_list[b], 0)
            token_lang_ids[b].fill_(base_lang_id)

            seq_len = int(lengths[b].item())
            seq_len = max(0, min(seq_len, L))
            if seq_len == 0:
                continue

            ratio = ratio_list[b] if ratio_list and b < len(ratio_list) else None
            if ratio is None or cs_first is None or cs_second is None:
                continue
            ...
            body_len = max(seq_len - 1, 0)  # exclude leading lang token
            if body_len <= 0:
                continue

            first_tokens = int(round(ratio_val * body_len))
            first_tokens = max(0, min(body_len, first_tokens))

            start = 1  # index 0 is reserved for language prefix token
            end = start + body_len
            if first_tokens > 0:
                token_lang_ids[b, start:start + first_tokens] = cs_first
            if first_tokens < body_len:
                token_lang_ids[b, start + first_tokens:end] = cs_second

        return token_lang_ids
    ...
    def collater_label(self, targets_by_label, audio_size, audio_starts, lang_list):
        ...
        for lang in lang_list:
            if lang == 'en':
                languages.append(1000)
            elif lang == 'it':
                languages.append(1001)
            elif lang == 'fr':
                languages.append(1002)
            elif lang == 'es':
