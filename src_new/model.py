...
class utut_seq2seq(FairseqEncoderDecoderModel):
    ...
    def forward(self, **kwargs): # 전체 모델 inference 때 사용되는 함수 #**kwargs = net_input임 (dictionary)
            ...
            decoder_lang_embed = lang_embed
            token_lang_ids = kwargs.get('token_languages', None)
            if token_lang_ids is None:
                token_lang_ids_list = kwargs.get('token_languages_list', None)
                if token_lang_ids_list:
                    token_lang_ids = token_lang_ids_list[0]
            if token_lang_ids is not None:
                token_lang_tensor = token_lang_ids.to(output['encoder_out'].device, dtype=torch.long)
                decoder_lang_embed = self.language_embedding(token_lang_tensor)
                if 'prev_output_tokens' in kwargs:
                    tgt_len = kwargs['prev_output_tokens'].size(-1)
                    if decoder_lang_embed.size(1) > tgt_len:
                        decoder_lang_embed = decoder_lang_embed[:, :tgt_len, :]
                    elif decoder_lang_embed.size(1) < tgt_len:
                        pad_len = tgt_len - decoder_lang_embed.size(1)
                        pad_tensor = decoder_lang_embed.new_zeros(decoder_lang_embed.size(0), pad_len, decoder_lang_embed.size(2))
                        decoder_lang_embed = torch.cat([decoder_lang_embed, pad_tensor], dim=1)

            output['decoder_lang_embed'] = decoder_lang_embed
