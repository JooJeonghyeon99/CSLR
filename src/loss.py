...
class LabelSmoothedCrossEntropyWithCtcCriterionConfig(
    ...
    ctc_weight: float = field(default=0.1, metadata={"help": "weight for CTC loss"})


@register_criterion(
    "label_smoothed_cross_entropy_with_joint_ctc",
...
class LabelSmoothedCrossEntropyWithCtcCriterion(LabelSmoothedCrossEntropyCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.ctc_weight > 0.0:
            ctc_lprobs, ctc_lens = model.get_ctc_output(encoder_out, sample)
